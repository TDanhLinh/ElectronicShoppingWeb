...
public interface ProductRepository extends JpaRepository<Product, Long>, JpaSpecificationExecutor<Product> {
    ...
    // // Lấy phần còn lại của danh sách `input.getArguments()` sau khi bỏ qua phần
    // tử đầu tiên
    ...
    // List.of(jsonPredicate));
    // Specification<Product> searchable = SearchUtils.parse(search,
    // SearchFields.CLIENT_PRODUCT);
    ...
    // Optional.ofNullable(filterable.toPredicate(root, query,
    // cb)).ifPresent(wheres::add);
    // Optional.ofNullable(searchable.toPredicate(root, query,
    // cb)).ifPresent(wheres::add);
